...
class FinanceApp:
    def __init__(self, root):
        ...
        self.ent_date.insert(0, "2026-01-04"); self.ent_date.grid(row=1, column=2, padx=5)

        self.btn_add = tk.Button(input_fr, text="Добавить", command=self.add_entry, bg="#e1f5fe", width=10)
        ...
        # --- Блок фильтров (Дата и Категория) ---
        ...
        tk.Button(filter_fr, text="Применить фильтр", command=self.refresh_table).grid(row=0, column=7, padx=5)
    ...
    def add_entry(self):
        ...
        if validate_amount(a) and validate_date(d) and c:
            ...
            self.refresh_table()
        else: messagebox.showwarning("Ввод", "Ошибка данных")
