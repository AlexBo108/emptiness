...
class FinanceApp:
    def __init__(self, root):
        ...
        self.btn_exp = tk.Button(input_fr, text="Добавить Расход", bg="#ffcdd2", command=lambda: self.process_entry("expense"))
        self.btn_exp.grid(row=3, column=3, padx=5)
        self.btn_inc = tk.Button(input_fr, text="Добавить Доход", bg="#c8e6c9", command=lambda: self.process_entry("income"))
        ...
        tk.Button(filter_fr, text="Применить", command=self.refresh_table).grid(row=0, column=7, padx=5)
        tk.Button(filter_fr, text="Сброс", command=self.reset_all_filters).grid(row=0, column=8, padx=5)

        # --- ТАБЛИЦА ---
        self.tree = ttk.Treeview(root, columns=("ID", "Sum", "Cat", "Date", "Comm", "Type"), show='headings')
        heads = [("ID", 50), ("Sum", 100), ("Cat", 150), ("Date", 100), ("Comm", 200), ("Type", 80)]
        for h, w in heads:
        ...
        # Кнопки аналитики справа снизу
        btn_bar = tk.Button(footer, text="Аналитика по месяцам", command=self.show_bar_chart)
        btn_bar.pack(side=tk.RIGHT, padx=5)
        btn_pie = tk.Button(footer, text="Круговая диаграмма", command=self.show_pie_chart)
        btn_pie.pack(side=tk.RIGHT, padx=5)
    ...
    def on_select(self, event):
        ...
        if sel:
            ...
            self.btn_exp.config(text="Сохранить Расход")
            self.btn_inc.config(text="Сохранить Доход")
    ...
    def process_entry(self, op_type):
        ...
        if validate_amount(a) and validate_date(d) and c:
            ...
            self.reset_ui()
        else: messagebox.showwarning("Ошибка", "Проверьте данные")
    ...
    def refresh_table(self):
        ...
        for r in rows:
            tag = 'over' if rem < 0 and r['op_type'] == 'expense' else ''
            self.tree.insert("", tk.END, values=(r['id'], r['amount'], r['category'], r['date'], r['comment'], r['op_type']), tags=(tag,))
